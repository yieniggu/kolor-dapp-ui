...
import { castVote, getProposals } from "../../../../store/slices/dao";
...
export const Proposal = () => {
  ...
  useEffect(() => {
    if (proposal) {
      ...
      const userVote = votes.find(
        ({ address: authorAddress }) => authorAddress === address
      );
  ...
  const sendVote = () => {
    ...
    dispatch(castVote({ option: selectedOption }, id, proposalId));
  };
                    ...
                    <div className="flex flex-col md:flex-row mt-6 gap-4">
                      <div className="rounded-full bg-interaction md:w-20 w-2/6 md:px-4 py-1">
                        <p className="text-center">
                          {new Date(proposal.endData) > new Date()
                            ? "Active"
                            : "Closed"}
                        </p>
                      </div>
